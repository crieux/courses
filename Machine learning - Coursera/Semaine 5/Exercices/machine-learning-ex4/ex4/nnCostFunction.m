...
J = pinv(-m) * sum(sum(y_new .* log(h_theta) + (1 - y_new) .* log(1 - h_theta)));
...
% Part III :
% Regularization 
Theta1_grad = pinv(m) * Theta1_grad + (lambda/m) * [zeros(size(Theta1,1), 1) Theta1(:,2:end)]; % for j >= 1 
Theta2_grad = pinv(m) * Theta2_grad + (lambda/m) * [zeros(size(Theta2,1), 1) Theta2(:,2:end)]; % for j >= 1
